...
//
using FileName = FileSystem::Path;
template<typename T>
class Property
{
public:
...
///*-----< overload insertion operator for vector properties>---*/
//inline std::ostream& operator<<(std::ostream& out, Property<std::vector<std::string>>& p) {
//	std::string temp = "";
//	std::vector<std::string> vec = p.getValue();
//	if (vec.size() > 0) {
...
/*-----< overload insertion operator for set properties>---*/
inline std::ostream& operator<<(std::ostream& out, Property<std::unordered_set<std::string>>& p) {
	std::string temp = "";
	std::unordered_set<std::string> vec = p.getValue();
	if (vec.size() > 0) {
		for (std::string elem : vec) {
			temp += (FileName::getName(elem) + " ");
		}
	}
	out << temp;
	return out;
