...
#include"../OODProject1/Demo/Help.h"
#include "DepAnal.h"
#include "TypeTable.h"
#include "TypeAnal.h"
...
using Dic = FileSystem::Directory;
...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
  ...
  try {
	...
	xmlPath = FileSystem::Path::getFullFileSpec(argv[argc - 1]);
	if (!FileSystem::Directory::exists(xmlPath)) {
		xmlPath= FileSystem::Path::getFullFileSpec("../Web.xml");
	}
...
void CodeAnalysis::CodeAnalysisExecutive::XmltoFile(std::string xml)
{
...
void CodeAnalysisExecutive::displayDataLines(ASTNode* pNode, bool isSummary)
{
  ...
  {
    if (pNode->parentType_ == "namespace" || pNode->parentType_ == "class" || pNode->parentType_ == "struct"||pNode->parentType_=="enum")
    {
...
{ using namespace CodeAnalysis;
  ...
  try {bool succeeded = exec.ProcessCommandLine(argc, argv);
	  ...
	  Rslt::write("\n");
	  exec.getSourceFiles();exec.processSourceCode(true);
	  exec.complexityAnalysis();exec.dispatchOptionalDisplays();
	  exec.flushLogger();exec.displayMetricSummary(50, 10);
	  exec.flushLogger();
	  Rslt::write("\n");std::ostringstream out;
	  ...
	  TypeAnal ta;//Start do type Anaylsis
	  ta.doTypeAnal();
	  DepAnal da(ta);
	  da.doDepAnal();
	  DepPublisher pu;
	  std::cout << "\n\n=======================Requirement 5#========================\n";
	  std::cout << "\nload the css file from ../style.css" << "\nload the js file from ../Hide.js\n";
	  std::cout << "\n=======================Requirement 6#========================\n";
	  std::cout << "In DepPublisher.cpp line 78:" << "<html><head><link rel=\"stylesheet\" type=\"text/css\" href=\"../style.css\"><script type=\"text/javascript\" src=\"../Hide.js\"></script></head><body><h3>";
	  pu.CreateIndex(da.getDB());
	  std::cout << "\n\n=======================Requirement 7#========================\n";
	  std::cout << "Using the Dependency Analysiser\n";
	  da.showDep();
	  std::cout << "\n\n=======================Requirement 3# 4# 9#========================\n";
	  std::cout << "open the web pages----including important packages & test files." << "\n";
		  std::string file_path = "..\\Web\\Index.htm";
