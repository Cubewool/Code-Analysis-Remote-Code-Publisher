...
//  Application:   Prototype for CSE687 Pr2, Sp17                  //
...
/*
  ...
  ====================
  ver 4.0 : 10 Mar 2017
  - Determin the the enum, typedef and alias rules.
  ver 3.3 : 26 Feb 2017
...
{  
  ...
  ///////////////////////////////////////////////////////////////
  // rule to detect enums statements
  ...
  public:
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Test enums definition: ", *pTc);
	  ...
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
		  ...
		  if (tc[tc.length() - 1] == ";") {
			  ...
			  if (len < tc.length())
				  ...
				  // this way you wont break other code, gauaranteed
				  // if you keep the check in doAction you will be 
				  // basically stopping your search and not allowing IRule::continue
				  if (tc.find("namespace") == tc.length()) {
					  doActions(pTc);
					  return IRule::Stop;
				  }
  ...
  public:
	  ...
	  void doAction(const Scanner::ITokCollection* pTc) override
	  {
		  ...
		  * because it is an typedef that cannot have children
		  */
		  p_Repos->scopeStack().top()->children_.push_back(pElem);
